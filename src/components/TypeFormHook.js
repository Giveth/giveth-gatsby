...
const TypeForm = props => {
  ...
  const goForward = evt => {
    if (current < props.fields.length) {
      ...
      const { label, formStyle } = props.fields.filter(o => o.name === name)[0]

      setCurrent(current + 1)
      addFormValue(value, type, name, label, formStyle)
    }
  ...
  const addFormValue = (value, type, name, label, formStyle) => {
    setFields(
      fields.concat({
        value,
        type,
